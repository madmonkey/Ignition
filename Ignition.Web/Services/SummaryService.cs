                         ...
                         ceg =>
                             {
                                 ...
                                 return summaryCategoryResponse != null ? new SummaryCategoryResponse()
                                        {
                                            Category = summaryCategoryResponse.Category,
                                            Number = ceg.Sum(i => 1),
                                            Total = g.Count(),
                                            Percentage = (ceg.Sum(i => 1) *100) / g.Count()
                                        } : null;
                             }).ToList();
                //Parallel.ForEach(summaryResponse.ByCategory, c => { c.Percentage = (c.Number / c.Total) * 100; });
                    ...
                    areas.Select(c => new SummaryLocationResponse
                        {
                            ...
                            Number = c.Number,
                            Total = c.Total,
                            Percentage = ((c.Number *100) / c.Total)
                        }).ToList();
                ///Parallel.ForEach(summaryResponse.ByLocation, l => { l.Percentage = (l.Number/l.Total)*100; });
            }
